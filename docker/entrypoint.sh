...
# SCORBOT_DISPLAY_MODE=native   run the command as-is, hosts display is used.
# SCORBOT_DISPLAY_MODE=none     run the command with no display at all.
...
if [ "$mode" = "vnc" ]; then
    ...
    # X server with an RFB port only reachable inside the container
