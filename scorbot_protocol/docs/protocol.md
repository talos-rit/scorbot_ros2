...
page adds the byte layer, timing, and behavior rules that the `.proto` cannot express.
...
and costs that one frame. This is by design: the next delimiter resynchronizes.
...
| Request response deadline | 100 ms (Home: immediate acknowledgment) |
